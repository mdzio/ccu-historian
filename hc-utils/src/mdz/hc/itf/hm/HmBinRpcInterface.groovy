...
/**
 * HmBinRpcInterface communicates with a HomeMatic interface process over BIN-RPC.
 * 
...
public class HmBinRpcInterface extends BasicProducer<RawEvent> implements Interface, WriteSupport, Consumer<RawEvent>, HmReinitable {
	...
	public synchronized void init() {
		client.call('init', [server.url, name])
		lastCommTime=[]
		registered=true
	}
		
	private synchronized void deinit() {
		client.call('init', [server.url])
		registered=false
	}
	...
	private void updateDeviceProperties(List<DataPoint> dps, long maxCacheAge) {
		dps.each { DataPoint dp ->
			...
			if (cachedProperties==null) {
				try {
					client.call('getParamsetDescription', [dp.id.address, PARAMSET_VALUES]).each { Map.Entry entry ->
						Map meta=(Map)entry.value
	...
	public synchronized void writeValue(DataPoint dp, value) {
		...
		client.call('setValue', [dp.id.address, dp.id.identifier, value])
	}
}
