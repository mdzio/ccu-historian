...
VERSION=$(curl -s https://api.github.com/repos/mdzio/ccu-historian/releases | jq -r '[to_entries[] | select(.value.prerelease)][0] | .value.tag_name')
echo "Version=$VERSION"

URL=$(curl -s https://api.github.com/repos/mdzio/ccu-historian/releases | jq -r '[to_entries[] | select(.value.prerelease)][0] | .value.assets[].browser_download_url' |grep .zip)
